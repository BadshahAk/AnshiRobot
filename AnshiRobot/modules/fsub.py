...
from AnshiRobot import BOT_ID
from AnshiRobot import DRAGONS as DEVS
from AnshiRobot import OWNER_ID
from AnshiRobot import telethn as Anshi
from AnshiRobot.events import Anshiinline
from AnshiRobot.events import register as Anshibot
from AnshiRobot.modules.no_sql import fsub_db as db


async def is_admin(chat_id, user_id):
    try:
        p = await Anshi(GetParticipantRequest(chat_id, user_id))
...
async def participant_check(channel, user_id):
    try:
        await Anshi(GetParticipantRequest(channel, int(user_id)))
        return True
    except UserNotParticipantError:
        return False
    except:
...
@Anshibot(pattern="^/(fsub|Fsub|forcesubscribe|Forcesub|forcesub|Forcesubscribe) ?(.*)")
