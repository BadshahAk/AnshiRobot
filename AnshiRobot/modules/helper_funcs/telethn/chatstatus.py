...
from AnshiRobot import DRAGONS
from AnshiRobot.modules.helper_funcs.telethn import IMMUNE_USERS, telethn
...
async def Anshi_is_admin(chat_id: int):
    status = False
    Anshi = await telethn.get_me()
    async for user in telethn.iter_participants(
        chat_id, filter=ChannelParticipantsAdmins
    ):
        if Anshi.id == user.id:
