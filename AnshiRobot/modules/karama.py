...
from AnshiRobot import OWNER_ID, pbot
from AnshiRobot.utils.admins import can_change_info
from AnshiRobot.utils.errors import capture_err
from AnshiRobot.utils.mongo import (
    alpha_to_int,
    get_karma,
    get_karmas,
    int_to_alpha,
...
async def downvote(_, message):
    ...
    await update_karma(chat_id, await int_to_alpha(user_id), new_karma)
    await message.reply_text(
        f"❖ ᴅᴇᴄʀᴇᴍᴇɴᴛᴇᴅ ᴋᴀʀᴍᴀ ᴏғ {user_mention} ʙʏ 1\n**● ᴛᴏᴛᴀʟ ᴩᴏɪɴᴛs ➥** {karma}\n\n❖ ᴘᴏᴡᴇʀᴇᴅ ʙʏ ➥ 𝓐𝓷𝓼𝓱𝓲 ༊·", reply_markup=InlineKeyboardMarkup(EVAA),
    )
