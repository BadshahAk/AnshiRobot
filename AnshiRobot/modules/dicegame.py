...
import asyncio
from AnshiRobot import pbot as Anshi
...
@Anshi.on_message(filters.command("dice"))
...
@Anshi.on_message(filters.command("dart"))
async def dart(bot, message):
    x=await bot.send_dice(message.chat.id, "🎯")
    m=x.dice.value
    await message.reply_text(f"⬤ ʜᴇʏ ʙᴀʙʏ ➥ {message.from_user.mention}\n⬤ ʏᴏᴜʀ sᴄᴏʀᴇ ɪs ➥ {m}", reply_markup=InlineKeyboardMarkup(EVAA),quote=True)

@Anshi.on_message(filters.command("basket"))
async def basket(bot, message):
    x=await bot.send_dice(message.chat.id, "🏀")
    m=x.dice.value
    await message.reply_text(f"⬤ ʜᴇʏ ʙᴀʙʏ ➥ {message.from_user.mention}\n⬤ ʏᴏᴜʀ sᴄᴏʀᴇ ɪs ➥ {m}",reply_markup=InlineKeyboardMarkup(EVAA), quote=True)
@Anshi.on_message(filters.command("jackpot"))
async def basket(bot, message):
    x=await bot.send_dice(message.chat.id, "🎰")
    m=x.dice.value
    await message.reply_text(f"⬤ ʜᴇʏ ʙᴀʙʏ ➥ {message.from_user.mention}\n⬤ ʏᴏᴜʀ sᴄᴏʀᴇ ɪs ➥ {m}",reply_markup=InlineKeyboardMarkup(EVAA), quote=True)
@Anshi.on_message(filters.command("ball"))
async def basket(bot, message):
    x=await bot.send_dice(message.chat.id, "🎳")
    m=x.dice.value
    await message.reply_text(f"⬤ ʜᴇʏ ʙᴀʙʏ ➥ {message.from_user.mention}\n⬤ ʏᴏᴜʀ sᴄᴏʀᴇ ɪs ➥ {m}",reply_markup=InlineKeyboardMarkup(EVAA), quote=True)
@Anshi.on_message(filters.command("football"))
async def basket(bot, message):
    x=await bot.send_dice(message.chat.id, "⚽")
    m=x.dice.value
    await message.reply_text(f"⬤ ʜᴇʏ ʙᴀʙʏ ➥ {message.from_user.mention}\n⬤ ʏᴏᴜʀ sᴄᴏʀᴇ ɪs ➥ {m}",reply_markup=InlineKeyboardMarkup(EVAA), quote=True)
